...
/// TCP Transport implementation with framing support
/// TCP is a stream protocol, so we need to add message framing
/// Each message is prefixed with a 4-byte length field (big-endian)
...
impl TcpTransport {
    ...
    pub async fn accept(&self) -> Result<TcpStream> {
        ...
        // Clear read buffer for new connection
        {
            let mut buffer = self.read_buffer.lock().await;
            buffer.clear();
        }

        // We need to return a reference, but we can't clone TcpStream
        // So we'll return an error for now and handle it differently
...
impl Transport for TcpTransport {
    ...
    fn local_addr(&self) -> Result<SocketAddr> {
        // Block briefly to get the address
        ...
        runtime.block_on(async {
            ...
            if let Some(stream) = stream_guard.as_ref() {
                ...
                anyhow::bail!("No socket available")
            }
        })
    }
}
